...
import { buildWriterPrompt } from "../prompts/legacy/writer";
import { AiInput } from "./build-ai-input";
import { PlannerOutput, TokenUsage, UsageLogEntry, StageUsage } from "../types";
import { logUsage } from "../usage-logger";

export async function writeDraft(input: AiInput, plan: PlannerOutput, wordRange: { min: number; max: number }): Promise<{ result: string; usage: TokenUsage; stageUsage: StageUsage } | null> {
  const client = getOpenAIClient();
  if (!client) return null;

  const model = getModelForStage("writer" as StageName);
  const { system, user } = buildWriterPrompt(input, plan, wordRange);
  ...
  try {
    ...
    const stageUsage: StageUsage = {
      stage: "writer", model, responseId, durationMs: duration,
    ...
    await logUsage({
      timestamp: new Date().toISOString(), model, pipelineStage: "writer",
      inputTokens: promptTokens, cachedInputTokens: cachedTokens,
      outputTokens: completionTokens, totalTokens, reasoningTokens,
